...
const Login = () => {
  ...
  const testAccounts = [
    { name: 'Admin (Duong)', user: 'admin_duong', desc: 'Full System Control' },
    { name: 'Editor (Minh)', user: 'editor_minh', desc: 'Articles & Products' },
    { name: 'Sales Agent (Lan)', user: 'sales_lan', desc: 'Orders & Customers' }
  ];

  const handleQuickSelect = (acc) => {
    setUsername(acc.user);
    setPassword('chinh123');
    setError('');
  };
      ...
      <div className="relative w-full max-w-[420px] z-10 animate-in fade-in zoom-in duration-300">
        <GlassCard hoverEffect={false} className="p-8 border border-white/10 shadow-2xl relative">
          ...
          <div className="mt-8 pt-6 border-t border-white/5 space-y-3">
            <span className="text-[9px] uppercase tracking-wider font-bold text-slate-500 block text-center">Quick Sandbox Authenticate</span>
            <div className="grid grid-cols-1 gap-2">
              {testAccounts.map((acc, idx) => (
                <button
                  ...
                  className="w-full p-2.5 rounded-lg border border-white/5 bg-white/[0.01] hover:border-purple-500/30 hover:bg-purple-500/5 transition-all text-left flex items-center justify-between group"
                >
                  <div>
                    <p className="text-[11px] font-bold text-slate-300 group-hover:text-white transition-colors">{acc.name}</p>
                    <p className="text-[9px] text-slate-500 font-mono">User: {acc.user}</p>
                  </div>
                  <span className="text-[9px] font-mono font-semibold text-purple-400 group-hover:underline">{acc.desc}</span>
                </button>
              ))}
            </div>
            <p className="text-[9px] text-center text-slate-600 font-medium">Default Sandbox Password: <code className="text-slate-400">chinh123</code></p>
          </div>
